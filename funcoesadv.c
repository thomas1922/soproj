...
pthread_mutex_t vcpu = PTHREAD_MUTEX_INITIALIZER;
...
void *vCPU(void *n){
	...
	while(1){
		...
		sem_post(write_mutex);
		pthread_cond_signal(&mem->edge_servers[va.index].vcpu_work[va.i]);
    ...
    while(1){
    	...
    	if(read(mem->edge_servers[index].fds[0], &t, sizeof(t))){
		    ...
		    if(mem->edge_servers[index].flag[0]==0){
    			...
    			pthread_cond_wait(&mem->edge_servers[index].vcpu_work[0], &vcpu_mutex);
    			printf("foise\n");
...
void *dispatcher_func(){
    ...
    while(1){
    	...
    	while(!mem->ativo){
    		pthread_mutex_lock(&vcpu);
    		printf("esperando sinal\n");
    		pthread_cond_wait(&vcpu_free, &vcpu);	
			printf("recebeu sinal\n");
			pthread_mutex_unlock(&vcpu);
...
void taskmanager(){
    ...
    while(1){ 
    	...
    	if(read(fd, &mob, sizeof(mob))>0) {
            ...
            if(!mem->ativo && areFrees()){
            	pthread_cond_signal(&vcpu_free);
            				printf("enviou sinal\n");
            }
			pthread_mutex_lock(&vcpu);
