...
void *vCPU(void *n){
	...
	if(va.i==0){
		...
		/*pthread_mutex_lock(&monitor_mutex);
			while(mem->flag!=1){
				pthread_cond_wait(&go_on, &monitor_mutex);
				printf("ativado\n");
			}*/
...
void *dispatcher_func(){
    ...
    while(1){
    	...
    	pthread_cond_broadcast(&dispatcher_work);
    	printf("chegouaqui\n");
...
void taskmanager(){
    ...
    while(1){ 
        ...
        //pthread_mutex_lock(&dispatcher_mutex);
        	...
        	else if(mem->ativo && areFrees()&& mem->tarefas.tamanho >0){
        		printf("true2\n");
        		sem_wait(write_mutex);
        		mem->keep_going_dispatcher=1;
        		sem_post(write_mutex);
